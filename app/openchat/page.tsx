...
export default function OpenChatPage() {
  ...
  if (currentRoom) return (
    <div className="fixed inset-0 flex flex-col bg-gray-50 z-[100]">
      ...
      {/* 메시지 목록 */}
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-2" onClick={() => { setCtxMenu(null); setShowPlusMenu(false); setShowSpecialMenu(false); }}>
        {messages.length === 0 && (
      ...
      <div className="px-3 py-2 bg-white border-t border-gray-100 flex items-center gap-2 shrink-0">
        ...
        <div className="relative shrink-0">
          ...
          <button onClick={() => { setShowPlusMenu((p) => !p); setShowSpecialMenu(false); }}
            className={`w-10 h-10 rounded-[12px] flex items-center justify-center transition shrink-0 text-xl font-black ${showPlusMenu ? "bg-sky-400 text-white" : "bg-sky-50 text-sky-500"}`}>
            {showPlusMenu ? "✕" : "+"}
          </button>
        </div>
