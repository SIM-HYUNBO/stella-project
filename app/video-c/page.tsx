...
export default function VideoCreator() {
  ...
  const saveToLocal = async () => {
    ...
    await saveVideoBlob(name, recordedBlob);
    await refreshSaved();
    alert(`저장 완료: ${name}`);
  };
  ...
  return (
    <div className="p-6 ml-11 max-w-3xl">
      <h1 className="text-2xl font-bold text-orange-900 dark:text-white mb-4">비디오 촬영</h1>

      {/* Camera */}
      <div className="mb-4">
        <video ref={videoRef} autoPlay playsInline muted style={{ width: 640, height: 480, background: "black" }} />
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <label className="flex items-center gap-2">
            ...
            <input type="number" min={0} value={endSec} onChange={e => setEndSec(Number(e.target.value))}
              className="w-20 border rounded px-2 py-1" />
          </label>
          <label className="flex items-center gap-2">
          ...
          <button onClick={applyTrimOrMute} disabled={processing || !recordedBlob}
           ...
           {/* Save */}
      <div className="mb-6">
        <button onClick={saveToLocal} className="px-4 py-2 bg-blue-500 text-white rounded">
          저장
        </button>
      </div>
            ...
            {saved.map((v) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => loadSavedForPreview(v.key)}
                    className="px-3 py-1 bg-gray-200 rounded"
                  >
                    불러와 미리보기
                  </button>
                  <button
                    onClick={() => removeSaved(v.key)}
