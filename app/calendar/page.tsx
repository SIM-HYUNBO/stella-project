  ...
  return (
    <PageContainer>
      <div className="flex flex-col h-[calc(100vh-48px)] bg-white -m-4 overflow-hidden">
        ...
        {/* ── 상단 헤더 ── */}
        <div className="shrink-0 px-4 pt-3 pb-0 bg-white border-b-2 border-gray-700">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={goToday} className="px-3 py-1.5 rounded-xl bg-sky-50 text-sky-500 text-xs font-black">오늘</button>
              {/* 월/일 뷰 토글 */}
              <div className="flex rounded-xl overflow-hidden border-2 border-gray-700">
                <button onClick={() => setViewMode("month")}
                  className={`px-3 py-1.5 text-xs font-black transition ${viewMode === "month" ? "bg-sky-500 text-white" : "bg-white text-gray-400"}`}>월</button>
                <button onClick={() => setViewMode("day")}
                  className={`px-3 py-1.5 text-xs font-black transition ${viewMode === "day" ? "bg-sky-500 text-white" : "bg-white text-gray-400"}`}>일</button>
              </div>
              {viewMode === "month" && <>
        ...
        {viewMode === "month" && (
          <div className="flex flex-col flex-1 overflow-hidden">
                ...
                return (
                  <button key={cell.key + i} onClick={() => setSelectedDate(cell.key)}
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition
                      ${isToday ? "bg-sky-500 text-white font-black shadow-md" :
            ...
            {/* 선택된 날 목록 */}
            <div className="flex-1 border-t-2 border-gray-700 bg-gray-50 overflow-y-auto">
              <div className="flex items-center justify-between px-5 py-3">
                <div className="flex items-baseline gap-1.5">
                  ...
                  {isSelToday && <span className="text-[10px] bg-sky-500 text-white px-1.5 py-0.5 rounded-full font-black">오늘</span>}
                </div>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setViewMode("day")}
                  ...
                  <button onClick={() => { setShowAdd(true); setNewTitle(""); setNewColor(EVENT_COLORS[0]); setNewAllDay(false); setNewStart("09:00"); setNewEnd("10:00"); }}
                    className="w-7 h-7 rounded-full bg-sky-500 text-white flex items-center justify-center shadow-md active:scale-90 transition">
        ...
        {viewMode === "day" && (
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* 날짜 네비 */}
            <div className="shrink-0 flex items-center justify-between px-4 py-2 bg-white border-b-2 border-gray-700">
            ...
            {(selectedHoliday || allDayEvents.length > 0) && (
              <div className="shrink-0 px-4 py-2 bg-white border-b-2 border-gray-700 space-y-1.5">
                {selectedHoliday && (
              ...
              <div className="relative" style={{ height: 24 * HOUR_HEIGHT }}>
                ...
                {isSelToday && (
                  <div className="absolute left-0 right-0 z-10 flex items-center" style={{ top: (nowMin / 60) * HOUR_HEIGHT }}>
                    <span className="w-14 text-right pr-2 text-[9px] text-sky-500 font-black shrink-0">
                      {String(Math.floor(nowMin / 60)).padStart(2,"0")}:{String(nowMin % 60).padStart(2,"0")}
                    </span>
                    <div className="w-2 h-2 rounded-full bg-sky-500 shrink-0 -ml-1" />
            ...
            <button
              ...
              className="absolute bottom-20 right-5 w-12 h-12 rounded-full bg-sky-500 text-white flex items-center justify-center shadow-xl active:scale-90 transition z-30"
            >
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
            <div className="w-full bg-white rounded-t-[28px] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              ...
              <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
                ...
                className="w-full h-12 rounded-2xl bg-gray-50 border-2 border-gray-700 px-4 text-sm outline-none text-[#1c1c1e] placeholder:text-gray-300 focus:ring-2 focus:ring-sky-200"/>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <input type="time" value={newStart} onChange={(e) => setNewStart(e.target.value)}
                      className="w-full h-10 rounded-xl bg-gray-50 border-2 border-gray-700 px-3 text-sm outline-none focus:ring-2 focus:ring-sky-200"/>
                  </div>
                  <div className="flex-1">
              ...
              <button onClick={addEvent} disabled={adding || !newTitle.trim()}
                className="w-full h-12 rounded-2xl bg-sky-500 text-white font-black shadow-md active:scale-95 transition disabled:opacity-50">
                추가
              </button>
            </div>
          </div>
