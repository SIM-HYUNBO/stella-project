  ...
  return (
    <PageContainer>
      <div className="flex flex-col h-[calc(100vh-48px)] bg-white -m-4 overflow-hidden">
        ...
        {/* ── 상단 헤더 ── */}
        <div className="shrink-0 px-4 pt-3 pb-0 bg-white border-b border-gray-300">
        ...
        {viewMode === "month" && (
          <div className="flex flex-col flex-1 overflow-hidden">
                ...
                return (
                  <button key={cell.key + i} onClick={() => setSelectedDate(cell.key)}
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition
                      ${isToday ? "bg-sky-100 text-sky-800 font-black shadow-md" :
            ...
            {/* 선택된 날 목록 */}
            <div className="flex-1 border-t border-gray-300 bg-gray-50 overflow-y-auto">
              <div className="flex items-center justify-between px-5 py-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => { setShowAdd(true); setNewTitle(""); setNewColor(EVENT_COLORS[0]); setNewAllDay(false); setNewStart("09:00"); setNewEnd("10:00"); }}
                    className="w-7 h-7 rounded-full bg-sky-100 text-sky-800 flex items-center justify-center shadow-md active:scale-90 transition">
        ...
        {viewMode === "day" && (
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* 날짜 네비 */}
            <div className="shrink-0 flex items-center justify-between px-4 py-2 bg-white border-b border-gray-300">
            ...
            {(selectedHoliday || allDayEvents.length > 0) && (
              <div className="shrink-0 px-4 py-2 bg-white border-b border-gray-300 space-y-1.5">
                {selectedHoliday && (
            ...
            <button
              ...
              className="absolute bottom-20 right-5 w-12 h-12 rounded-full bg-sky-100 text-sky-800 flex items-center justify-center shadow-xl active:scale-90 transition z-30"
            >
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
            <div className="w-full bg-white rounded-t-[28px] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              ...
              <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
                ...
                className="w-full h-12 rounded-2xl bg-gray-50 border border-gray-300 px-4 text-sm outline-none text-[#1c1c1e] placeholder:text-gray-300 focus:ring-2 focus:ring-sky-200"/>

              {/* 종일 토글 */}
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-gray-600">종일</span>
                <button onClick={() => setNewAllDay(!newAllDay)}
                  className={`relative w-12 h-6 rounded-full transition-colors ${newAllDay ? "bg-sky-100" : "bg-gray-200"}`}>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <input type="time" value={newStart} onChange={(e) => setNewStart(e.target.value)}
                      className="w-full h-10 rounded-xl bg-gray-50 border border-gray-300 px-3 text-sm outline-none focus:ring-2 focus:ring-sky-200"/>
                  </div>
                  <div className="flex-1">
              ...
              <button onClick={addEvent} disabled={adding || !newTitle.trim()}
                className="w-full h-12 rounded-2xl bg-sky-100 text-sky-800 font-black shadow-md active:scale-95 transition disabled:opacity-50">
                추가
              </button>
            </div>
          </div>
