        ...
        <div className="shrink-0 px-4 pt-3 pb-0 bg-white">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={goToday} className="px-3 py-1.5 rounded-xl bg-yellow-50 text-sky-500 text-xs font-black">오늘</button>
              {/* 월/일 뷰 토글 */}
              <div className="flex rounded-xl overflow-hidden">
                <button onClick={() => setViewMode("month")}
                  className={`px-3 py-1.5 text-xs font-black transition ${viewMode === "month" ? "bg-yellow-500 text-white" : "bg-white text-gray-400"}`}>월</button>
                <button onClick={() => setViewMode("day")}
                  className={`px-3 py-1.5 text-xs font-black transition ${viewMode === "day" ? "bg-yellow-500 text-white" : "bg-white text-gray-400"}`}>일</button>
              </div>
              {viewMode === "month" && <>
                ...
                return (
                  <button key={cell.key + i} onClick={() => setSelectedDate(cell.key)}
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition
                      ${isToday ? "bg-yellow-500 text-white font-black" :
                        isSelected ? "bg-sky-100 text-sky-600" :
            ...
            <div className="flex-1 bg-gray-50 overflow-y-auto">
              <div className="flex items-center justify-between px-5 py-3">
                <div className="flex items-baseline gap-1.5">
                  ...
                  {isSelToday && <span className="text-[10px] bg-yellow-500 text-white px-1.5 py-0.5 rounded-full font-black">오늘</span>}
                </div>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setViewMode("day")}
                  ...
                  <button onClick={() => { setShowAdd(true); setNewTitle(""); setNewColor(EVENT_COLORS[0]); setNewAllDay(false); setNewStart("09:00"); setNewEnd("10:00"); }}
                    className="w-7 h-7 rounded-full bg-yellow-500 text-white flex items-center justify-center active:scale-90 transition">
        ...
        {viewMode === "day" && (
          <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="relative" style={{ height: 24 * HOUR_HEIGHT }}>
                ...
                {isSelToday && (
                  <div className="absolute left-0 right-0 z-10 flex items-center" style={{ top: (nowMin / 60) * HOUR_HEIGHT }}>
                    ...
                    <div className="w-2 h-2 rounded-full bg-yellow-500 shrink-0 -ml-1" />
            ...
            <button
              ...
              className="absolute bottom-20 right-5 w-12 h-12 rounded-full bg-yellow-500 text-white flex items-center justify-center active:scale-90 transition z-30"
            >
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
            <div className="w-full bg-white rounded-t-[28px] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              ...
              <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
                ...
                className="w-full h-12 rounded-2xl bg-gray-50 px-4 text-sm outline-none text-[#1c1c1e] placeholder:text-gray-300 focus:ring-2 focus:ring-orange-200"/>
              ...
              <button onClick={addEvent} disabled={adding || !newTitle.trim()}
                className="w-full h-12 rounded-2xl bg-sky-100 text-white font-black active:scale-95 transition disabled:opacity-50">
                추가
              </button>
            </div>
          </div>
