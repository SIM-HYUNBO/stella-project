...
function GalagaGame({ audioCtxRef, onExit }:{ audioCtxRef:React.MutableRefObject<AudioContext|null>; onExit:()=>void }) {
  ...
  const addPopup=useCallback((x:number,y:number,text:string,color:string)=>{
    G.current.popups.push({id:G.current.popId++,x,y,vy:-1.5,life:45,text,color});
  },[]);
  ...
  const explode=useCallback((x:number,y:number,color:string,count=10)=>{
    for(let i=0;i<count;i++){
      ...
      G.current.particles.push({x,y,vx:Math.cos(a)*spd,vy:Math.sin(a)*spd,life:30+Math.random()*25,color,size:1.5+Math.random()*2});
    }
  },[]);

  const spawnEnemies=useCallback((wave:number,CW:number)=>{
    const g=G.current;
    const spX=(CW-ENEMY_COLS*40)/2;
    g.enemies=[];
    for(let row=0;row<ENEMY_ROWS;row++)
      for(let col=0;col<ENEMY_COLS;col++){
        const tx=spX+col*40+20, ty=48+row*38;
        g.enemies.push({
          id:row*ENEMY_COLS+col, col, row,
          x:tx, y:-50, tx, ty,
          alive:true, entering:true,
          entryDelay:col*6+row*5,
          diving:false, diveVx:0, diveVy:0, diveReturnX:tx, diveReturnY:ty,
        });
      }
    g.fmDir=1; g.fmSpeed=0.45+wave*0.07; g.fmDropPending=false;
    g.entering=true;
    g.nextDive=performance.now()+5000;
    g.nextEnemyShoot=performance.now()+3500;
  },[]);

  const spawnBoss=useCallback((wave:number,CW:number)=>{
    const g=G.current;
    const hp=10+Math.floor(wave/3)*5;
    g.boss={x:CW/2,y:70,hp,maxHp:hp,timer:0,nextShoot:performance.now()+2000,phase:0,invincible:0,alive:true};
    g.enemies=[]; g.entering=false;
    g.nextEnemyShoot=9e9;
  },[]);
  ...
  const draw=useCallback((now:number)=>{
    ...
    for(let i=0;i<90;i++){
      const layer=i%3;
      const speed=[0.12,0.35,0.9][layer];
      const sz=[0.8,1.3,2.2][layer];
      const bright=layer===2?0.5+0.4*Math.sin(f*0.04+i):0.15+0.15*(i%3);
      const sx=(i*137)%W;
      const sy=((i*97+f*speed)%H+H)%H;
    ...
    if(g.boss&&g.boss.alive){
      const boss=g.boss;
      const bw=Math.round(W*0.6), bx=(W-bw)/2;
      ctx.fillStyle="rgba(255,0,0,0.18)"; ctx.fillRect(bx,6,bw,9);
      const hpFrac=boss.hp/boss.maxHp;
      const hpColor=hpFrac>0.5?"#ff4444":hpFrac>0.25?"#ff8800":"#ffff00";
      ctx.fillStyle=hpColor; ctx.shadowBlur=8; ctx.shadowColor=hpColor;
      ctx.fillRect(bx,6,Math.round(bw*hpFrac),9);
      ctx.strokeStyle="rgba(255,100,100,0.4)"; ctx.lineWidth=1; ctx.strokeRect(bx,6,bw,9);
      ctx.font="bold 10px monospace"; ctx.textAlign="left"; ctx.fillStyle="rgba(255,150,150,0.7)";
      ctx.shadowBlur=0; ctx.fillText("BOSS",bx+3,14);
      // Body
      const bx2=boss.x, by2=boss.y;
      const pulse=1+0.05*Math.sin(f*0.08);
      const bc=boss.invincible>0?"#ffffff":"#ff3333";
      ctx.shadowBlur=22; ctx.shadowColor=bc; ctx.strokeStyle=bc; ctx.lineWidth=2.2;
      ctx.beginPath();
      ctx.moveTo(bx2,by2-28*pulse);
      ctx.lineTo(bx2+32,by2+14); ctx.lineTo(bx2+20,by2+8);
      ctx.lineTo(bx2+14,by2+24); ctx.lineTo(bx2,by2+18);
      ctx.lineTo(bx2-14,by2+24); ctx.lineTo(bx2-20,by2+8);
      ctx.lineTo(bx2-32,by2+14); ctx.closePath(); ctx.stroke();
      ctx.strokeStyle="rgba(255,100,100,0.5)"; ctx.lineWidth=1;
      ctx.beginPath(); ctx.moveTo(bx2,by2-16); ctx.lineTo(bx2+14,by2+8); ctx.lineTo(bx2-14,by2+8); ctx.closePath(); ctx.stroke();
      ctx.fillStyle=bc; ctx.shadowBlur=10;
      ctx.beginPath(); ctx.arc(bx2-6,by2-5,3.5,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(bx2+6,by2-5,3.5,0,Math.PI*2); ctx.fill();
      ctx.shadowBlur=0;
    }
    ...
    g.enemies.forEach(e=>{
      ...
      ctx.shadowBlur=12; ctx.shadowColor=color; ctx.strokeStyle=color; ctx.lineWidth=1.6;
      ctx.beginPath();
      ctx.moveTo(e.x,e.y-11); ctx.lineTo(e.x+11,e.y+7); ctx.lineTo(e.x+6,e.y+3);
      ctx.lineTo(e.x,e.y+9); ctx.lineTo(e.x-6,e.y+3); ctx.lineTo(e.x-11,e.y+7); ctx.closePath(); ctx.stroke();
    ...
    if(p.invincible<1||Math.floor(now/80)%2===0){
      ...
      ctx.lineTo(p.x,p.y+8); ctx.lineTo(p.x-5,p.y+5); ctx.lineTo(p.x-12,p.y+10); ctx.closePath(); ctx.stroke();
      if(g.powerType==="double"){
        ctx.strokeStyle="rgba(255,255,0,0.5)"; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(p.x-4,p.y-12); ctx.lineTo(p.x-4,p.y-20); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(p.x+4,p.y-12); ctx.lineTo(p.x+4,p.y-20); ctx.stroke();
      }
      if(g.powerType==="shield"){
        const fr=Math.max(0,(g.powerEnd-now)/10000);
        ctx.strokeStyle=`rgba(100,200,255,${0.3+0.3*Math.sin(f*0.1)})`; ctx.lineWidth=1.5;
        ctx.beginPath(); ctx.arc(p.x,p.y,22,0,Math.PI*2*fr); ctx.stroke();
      }
      const flameA=0.5+0.5*Math.sin(f*0.25);
      ctx.strokeStyle=`rgba(0,200,255,${flameA})`; ctx.lineWidth=1.5;
    ...
    g.powerups.forEach(pu=>{
      const pulse=1+0.18*Math.sin(f*0.12+pu.id);
      ctx.textAlign="center";
      if(pu.type==="double"){
        ctx.shadowBlur=14; ctx.shadowColor="#ffff00"; ctx.strokeStyle="#ffff00"; ctx.lineWidth=1.5;
        ctx.beginPath(); ctx.roundRect(pu.x-12,pu.y-10,24,20,6); ctx.stroke();
        ctx.fillStyle="#ffff00"; ctx.font="bold 12px monospace"; ctx.fillText("×2",pu.x,pu.y+4);
      } else if(pu.type==="shield"){
        ctx.shadowBlur=14; ctx.shadowColor="#44aaff"; ctx.strokeStyle="#44aaff"; ctx.lineWidth=1.5;
        ctx.beginPath(); ctx.arc(pu.x,pu.y,11*pulse,0,Math.PI*2); ctx.stroke();
        ctx.fillStyle="rgba(68,170,255,0.25)"; ctx.fill();
        ctx.fillStyle="#44aaff"; ctx.font="bold 12px monospace"; ctx.fillText("S",pu.x,pu.y+4);
      } else {
        ctx.shadowBlur=14; ctx.shadowColor="#ff6600"; ctx.fillStyle="#ff6600";
        ctx.beginPath(); ctx.arc(pu.x,pu.y,10*pulse,0,Math.PI*2); ctx.fill();
        ctx.fillStyle="white"; ctx.font="bold 11px monospace"; ctx.fillText("B",pu.x,pu.y+4);
      }
      ctx.shadowBlur=0;
    });
    ...
    g.bullets.forEach(b=>{
      if(b.fromPlayer){
        ctx.shadowBlur=10; ctx.shadowColor="#ffff44"; ctx.strokeStyle="#ffff44"; ctx.lineWidth=2.5;
        ctx.beginPath(); ctx.moveTo(b.x,b.y+10); ctx.lineTo(b.x,b.y-2); ctx.stroke();
      } else {
        ctx.shadowBlur=8; ctx.shadowColor="#ff4040"; ctx.strokeStyle="#ff6060"; ctx.lineWidth=1.5;
        ctx.beginPath(); ctx.moveTo(b.x,b.y-8); ctx.lineTo(b.x+b.vx*3,b.y+4); ctx.stroke();
      }
      ctx.shadowBlur=0;
    });
    ...
    g.particles.forEach(pt=>{
      const a=pt.life/55;
      ctx.fillStyle=pt.color+Math.round(Math.min(1,a)*200).toString(16).padStart(2,"0");
    ...
    g.popups.forEach(pop=>{
      const a=Math.min(1,pop.life/20);
      ctx.globalAlpha=a; ctx.font="bold 14px monospace"; ctx.fillStyle=pop.color;
      ctx.shadowBlur=8; ctx.shadowColor=pop.color;
    ...
    // Combo
    if(g.combo>=3){
      const ca=Math.min(1,(g.combo-2)*0.25);
      ctx.font=`bold ${Math.min(13+g.combo,22)}px monospace`;
      ctx.textAlign="center"; ctx.fillStyle=`rgba(255,220,0,${ca})`;
      ctx.shadowBlur=14; ctx.shadowColor="#ffaa00";
      ctx.fillText(`COMBO ×${g.combo}!`,W/2,H/2-10);
      ctx.shadowBlur=0;
    }

    // HUD
    ctx.shadowBlur=8; ctx.shadowColor="#ff00ff";
    ctx.font="bold 15px monospace"; ctx.textAlign="left"; ctx.fillStyle="#ff88ff";
    ctx.fillText(`${g.score}`,8,22);
    ctx.shadowColor="#44ffff"; ctx.textAlign="right"; ctx.fillStyle="#44ffff";
    ctx.fillText(`WAVE ${g.wave}`,W-8,22);
    // Power timer
    if(g.powerType!=="none"&&g.powerEnd>now){
      const sec=Math.ceil((g.powerEnd-now)/1000);
      const col=g.powerType==="double"?"#ffff44":"#44aaff";
      ctx.shadowColor=col; ctx.textAlign="center"; ctx.fillStyle=col;
      ctx.font="bold 11px monospace"; ctx.fillText(g.powerType==="double"?`×2 ${sec}s`:`SLD ${sec}s`,W/2,H-26);
    }
    // Lives
    for(let i=0;i<g.lives;i++){
      ctx.fillStyle="#00ffff"; ctx.shadowColor="#00ffff"; ctx.shadowBlur=8;
      const lx=W/2-20+i*20, ly=H-12;
      ctx.beginPath(); ctx.moveTo(lx,ly-7); ctx.lineTo(lx+7,ly+4); ctx.lineTo(lx,ly+1); ctx.lineTo(lx-7,ly+4); ctx.closePath(); ctx.fill();
    }
    ctx.shadowBlur=0;

    // Flash
    if(g.flash>0){
      ctx.fillStyle=`rgba(255,255,255,${Math.min(1,g.flash)})`;
  ...
  const loop=useCallback(()=>{
    ...
    if(g.entering){
      ...
      g.enemies.forEach(e=>{
        ...
        if(g.frame<e.entryDelay+5){ e.y=-50; e.x=e.tx; any=true; return; }
        const t=Math.min(1,(g.frame-e.entryDelay-5)/55);
        e.y=-50+(e.ty+50)*t;
        e.x=e.tx+Math.sin(t*Math.PI*3)*35*(1-t);
        if(t>=1){ e.entering=false; e.x=e.tx; e.y=e.ty; e.diveReturnX=e.tx; e.diveReturnY=e.ty; }
        else any=true;
      });
      if(!any){ g.entering=false; g.nextDive=now+1500; }
    }

    // Formation
    if(!g.entering){
      const alive=g.enemies.filter(e=>e.alive&&!e.diving&&!e.entering);
      if(alive.length>0){
        const minX=Math.min(...alive.map(e=>e.x)), maxX=Math.max(...alive.map(e=>e.x));
        if(maxX>=W-20){ g.fmDir=-1; g.fmDropPending=true; }
        if(minX<=20)  { g.fmDir=1;  g.fmDropPending=true; }
        const drop=g.fmDropPending?14:0;
        g.enemies.forEach(e=>{ if(!e.alive||e.diving||e.entering) return; e.x+=g.fmDir*g.fmSpeed; e.y+=drop; e.diveReturnX=e.x; e.diveReturnY=e.y; });
        if(g.fmDropPending) g.fmDropPending=false;
      }
    }

    // Dive
    if(!g.entering&&now>g.nextDive){
      const cands=g.enemies.filter(e=>e.alive&&!e.diving);
      if(cands.length>0){
        const diver=cands[Math.floor(Math.random()*cands.length)];
        diver.diving=true;
        const dx=g.player.x-diver.x, dy=g.player.y-diver.y, len=Math.hypot(dx,dy)||1;
        diver.diveVx=dx/len*4.5; diver.diveVy=dy/len*4.5;
      }
      g.nextDive=now+1800+Math.random()*1800;
    }
    g.enemies.forEach(e=>{
      if(!e.alive||!e.diving) return;
      e.x+=e.diveVx; e.y+=e.diveVy; e.diveVy+=0.07;
      if(e.y>H+35){ e.x=e.diveReturnX; e.y=-35; e.diveVy=-3.5; }
      if(Math.abs(e.x-e.diveReturnX)<18&&Math.abs(e.y-e.diveReturnY)<18&&e.y>e.diveReturnY-12)
        { e.diving=false; e.x=e.diveReturnX; e.y=e.diveReturnY; }
    });

    // Enemy shoot
    if(now>g.nextEnemyShoot&&!g.entering){
      const sh=g.enemies.filter(e=>e.alive);
      if(sh.length>0){
        const src=sh[Math.floor(Math.random()*sh.length)];
        const dx=g.player.x-src.x, dy=g.player.y-src.y, len=Math.hypot(dx,dy)||1;
        g.bullets.push({id:g.bulletId++,x:src.x,y:src.y+12,vx:dx/len*2.5,vy:dy/len*3.5+1,fromPlayer:false});
      }
      g.nextEnemyShoot=now+Math.max(400,1000-g.wave*40);
    }
    ...
    if(g.boss&&g.boss.alive){
      const boss=g.boss;
      boss.timer++;
      boss.x=W/2+Math.sin(boss.timer*0.022)*(W/2-55);
      if(boss.invincible>0) boss.invincible--;
      if(now>boss.nextShoot){
        const ph=boss.phase%4;
        const px=g.player.x, py=g.player.y;
        if(ph===0){
          for(let i=-2;i<=2;i++){const a=Math.PI/2+i*0.28;g.bullets.push({id:g.bulletId++,x:boss.x,y:boss.y+28,vx:Math.cos(a)*3.5,vy:Math.sin(a)*3.5,fromPlayer:false});}
          boss.nextShoot=now+1800;
        } else if(ph===1){
          const dx=px-boss.x, dy=py-boss.y, len=Math.hypot(dx,dy)||1;
          const perp={x:-dy/len,y:dx/len};
          for(let i=-1;i<=1;i++) g.bullets.push({id:g.bulletId++,x:boss.x+perp.x*i*12,y:boss.y+28,vx:dx/len*4+perp.x*i*0.5,vy:dy/len*4,fromPlayer:false});
          boss.nextShoot=now+2000;
        } else if(ph===2){
          for(let i=0;i<7;i++) g.bullets.push({id:g.bulletId++,x:boss.x+(i-3)*24,y:boss.y+28,vx:(i-3)*0.3,vy:4,fromPlayer:false});
          boss.nextShoot=now+2200;
        } else {
          for(let i=0;i<10;i++){const a=(i/10)*Math.PI*2+boss.timer*0.06;g.bullets.push({id:g.bulletId++,x:boss.x,y:boss.y,vx:Math.cos(a)*3,vy:Math.sin(a)*3,fromPlayer:false});}
          boss.nextShoot=now+1600;
        }
        boss.phase++;
      }
      ...
      g.bullets=g.bullets.filter(pb=>{
        ...
        if(Math.abs(boss.x-pb.x)<32&&Math.abs(boss.y-pb.y)<32&&boss.invincible===0){
          boss.hp--; boss.invincible=18; g.flash=0.2;
          explode(pb.x,pb.y,"#ff4444",6);
          try{playClick(getAudio(),SND_HIT,0.9);}catch(_){}
          if(boss.hp<=0){
            boss.alive=false; g.flash=0.8; g.shake=35;
            for(let i=0;i<5;i++) explode(boss.x+(Math.random()*70-35),boss.y+(Math.random()*50-25),"#ff4444",12);
            const pts=5000+g.wave*500; g.score+=pts;
            g.combo++; if(g.combo>g.maxCombo) g.maxCombo=g.combo;
            addPopup(boss.x,boss.y-30,`+${pts}`,"#ff4444");
            try{playClick(getAudio(),SND_DIE,0.5);}catch(_){}
            g.boss=null;
            const nw=g.wave+1; g.wave=nw;
            setTimeout(()=>{ if(!g.running) return; const cw=canvasRef.current?.width??340; if(isBossWave(nw)) spawnBoss(nw,cw); else spawnEnemies(nw,cw); },2000);
          }
          return false;
        }
        return true;
    ...
    // Player auto shoot
    if(now>g.nextShoot){
      const interval=g.powerType==="double"?300:460;
      if(g.powerType==="double"){
        g.bullets.push({id:g.bulletId++,x:g.player.x-5,y:g.player.y-14,vx:0,vy:-10,fromPlayer:true});
        g.bullets.push({id:g.bulletId++,x:g.player.x+5,y:g.player.y-14,vx:0,vy:-10,fromPlayer:true});
      } else {
        g.bullets.push({id:g.bulletId++,x:g.player.x,y:g.player.y-14,vx:0,vy:-10,fromPlayer:true});
      }
      g.nextShoot=now+interval;
      try{playClick(getAudio(),SND_SHOOT,0.8+Math.random()*0.1);}catch(_){}
    }

    // Bullet movement
    g.bullets.forEach(b=>{ b.y+=b.vy; b.x+=b.vx; });
    g.bullets=g.bullets.filter(b=>b.y>-25&&b.y<H+25&&b.x>-25&&b.x<W+25);

    // Player bullet vs enemies
    g.bullets=g.bullets.filter(pb=>{
      if(!pb.fromPlayer) return true;
      const hit=g.enemies.find(e=>e.alive&&!e.entering&&Math.abs(e.x-pb.x)<14&&Math.abs(e.y-pb.y)<14);
      if(hit){
        hit.alive=false;
        const pts=hit.diving?200:100; g.score+=pts;
        g.combo++; if(g.combo>g.maxCombo) g.maxCombo=g.combo;
        explode(hit.x,hit.y,ENEMY_COLORS[hit.row]);
        addPopup(hit.x,hit.y-14,`+${pts}`,ENEMY_COLORS[hit.row]);
        if(Math.random()<0.18){
          const types:Array<"double"|"shield"|"bomb">=["double","shield","bomb"];
          g.powerups.push({id:g.pupId++,x:hit.x,y:hit.y,vy:1.8,type:types[Math.floor(Math.random()*3)]});
        }
        try{playClick(getAudio(),SND_HIT,1+Math.random()*0.1);}catch(_){}
        return false;
      }
      return true;
    });

    // Enemy bullet/diver vs player
    if(g.player.invincible===0&&g.powerType!=="shield"){
      const hitB=g.bullets.find(b=>!b.fromPlayer&&Math.abs(b.x-g.player.x)<13&&Math.abs(b.y-g.player.y)<13);
      const hitD=g.enemies.find(e=>e.alive&&e.diving&&Math.abs(e.x-g.player.x)<18&&Math.abs(e.y-g.player.y)<18);
      if(hitB||hitD){
        if(hitB) g.bullets=g.bullets.filter(b=>b!==hitB);
        if(hitD){ hitD.alive=false; explode(hitD.x,hitD.y,ENEMY_COLORS[hitD.row]); }
        g.lives--; g.combo=0; g.player.invincible=140; g.shake=25; g.flash=0.45;
        explode(g.player.x,g.player.y,"#00ffff",14);
        try{playClick(getAudio(),SND_DIE);}catch(_){}
        if(g.lives<=0){ g.running=false; setFinal({score:g.score,wave:g.wave,maxCombo:g.maxCombo}); setPhase("over"); return; }
      }
    }

    // Power-up movement & pickup
    g.powerups.forEach(pu=>{ pu.y+=pu.vy; });
    const collected:GPowerUp[]=[];
    g.powerups=g.powerups.filter(pu=>{
      if(pu.y>H+20) return false;
      if(Math.abs(pu.x-g.player.x)<22&&Math.abs(pu.y-g.player.y)<22){ collected.push(pu); return false; }
      return true;
    });
    collected.forEach(pu=>{
      ...
      if(pu.type==="bomb"){
        ...
        let cleared=0;
        g.enemies.forEach(e=>{ if(e.alive){ e.alive=false; explode(e.x,e.y,ENEMY_COLORS[e.row],6); g.score+=50; cleared++; } });
        addPopup(g.player.x,g.player.y-30,cleared>0?`BOMB +${cleared*50}`:"BOMB CLEAR!","#ff6600");
        g.flash=0.5; g.shake=20;
      } else if(pu.type==="shield"){
        g.powerType="shield"; g.powerEnd=now+10000;
        addPopup(g.player.x,g.player.y-30,"SHIELD!","#44aaff");
      } else {
        g.powerType="double"; g.powerEnd=now+15000;
        addPopup(g.player.x,g.player.y-30,"DOUBLE SHOT!","#ffff44");
      }
    });

    // Wave clear
    if(!g.boss&&g.enemies.every(e=>!e.alive)&&!g.entering){
      const nw=g.wave+1; g.wave=nw;
      if(isBossWave(nw)) spawnBoss(nw,W);
      else spawnEnemies(nw,W);
    }

    // Enemies reach bottom
    if(g.enemies.some(e=>e.alive&&!e.entering&&e.y>H-40)){
      g.running=false; setFinal({score:g.score,wave:g.wave,maxCombo:g.maxCombo}); setPhase("over"); return;
    }

    // Particles + popups decay
    g.particles.forEach(pt=>{ pt.x+=pt.vx; pt.y+=pt.vy; pt.vy+=0.07; pt.life--; });
    g.particles=g.particles.filter(pt=>pt.life>0);
    g.popups.forEach(pop=>{ pop.y+=pop.vy; pop.life--; });
  ...
  const startGame=useCallback(()=>{
    ...
    const canvas=canvasRef.current; if(!canvas) return;
    const W=canvas.width, H=canvas.height;
    g.player={x:W/2,y:H-32,invincible:0};
    g.bullets=[]; g.particles=[]; g.powerups=[]; g.popups=[]; g.boss=null;
    g.score=0; g.combo=0; g.maxCombo=0; g.lives=3; g.wave=1;
    g.powerType="none"; g.powerEnd=0; g.shake=0; g.flash=0; g.frame=0;
    g.bulletId=0; g.pupId=0; g.popId=0; g.nextShoot=0;
    g.running=true;
    spawnEnemies(1,W);
    setPhase("playing");
    g.frameId=requestAnimationFrame(loop);
  ...
  useEffect(()=>{
    ...
    canvas.height=Math.round(canvas.width*1.45);
    const ctx=canvas.getContext("2d"); if(!ctx) return;
    ctx.fillStyle="#030008"; ctx.fillRect(0,0,canvas.width,canvas.height);
    for(let i=0;i<80;i++){
      ctx.fillStyle=`rgba(255,255,255,${0.1+0.4*(i%3)/3})`;
      ctx.fillRect((i*137)%canvas.width,(i*97)%canvas.height,[0.8,1.3,2][i%3],[0.8,1.3,2][i%3]);
    }
    ctx.shadowBlur=18; ctx.shadowColor="#ff00ff"; ctx.font="bold 24px monospace"; ctx.textAlign="center"; ctx.fillStyle="#ff88ff";
    ctx.fillText("NEON GALAGA",canvas.width/2,canvas.height/2-18);
    ctx.shadowBlur=8; ctx.shadowColor="#44ffff"; ctx.font="10px monospace"; ctx.fillStyle="rgba(100,220,255,0.55)";
    ctx.fillText("← A / → D  |  터치 버튼",canvas.width/2,canvas.height/2+8);
    ctx.shadowBlur=0;
    const px=canvas.width/2, py=canvas.height/2+46;
    ctx.shadowBlur=16; ctx.shadowColor="#00ffff"; ctx.strokeStyle="#00ffff"; ctx.lineWidth=2;
    ctx.beginPath(); ctx.moveTo(px,py-12); ctx.lineTo(px+11,py+8); ctx.lineTo(px,py+4); ctx.lineTo(px-11,py+8); ctx.closePath(); ctx.stroke();
    ctx.shadowBlur=0;
  },[]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12,width:"100%"}}>
      ...
      {phase==="over"&&(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
          <div style={{fontFamily:"monospace",textAlign:"center",lineHeight:1.8}}>
            <div style={{fontSize:26,fontWeight:900,color:"#ffff55",textShadow:"0 0 16px #ffff00"}}>{final.score.toLocaleString()}</div>
            <div style={{fontSize:12,color:"#80ffff"}}>WAVE {final.wave}</div>
            {final.maxCombo>=3&&<div style={{fontSize:11,color:"#ffcc44"}}>MAX COMBO ×{final.maxCombo}</div>}
          </div>
      ...
      {phase==="playing"&&(
        <div style={{display:"flex",gap:16,width:"100%",maxWidth:CW}}>
          <button
            ...
            style={{flex:1,height:68,borderRadius:16,background:"rgba(0,200,255,0.08)",border:"1.5px solid rgba(0,255,255,0.45)",boxShadow:"0 0 18px rgba(0,200,255,0.2)",cursor:"pointer",touchAction:"none",fontSize:30,color:"#44ffff"}}>◀</button>
          <button
      ...
      <button onClick={onExit} style={{fontSize:12,color:"rgba(255,100,255,0.35)",background:"none",border:"none",cursor:"pointer"}}>← 키캡으로</button>
    </div>
  );
}
