    ...
    <main className="relative min-h-screen overflow-hidden" onClick={() => setMenuOpen(null)}>
      ...
      <div className="relative z-10">
        ...
        <div className="px-5 pt-4 pb-20 space-y-5">
          ...
          {requests.length > 0 && (
            <div>
              <p className="font-black text-slate-800 text-base mb-3 px-1">받은 요청 🔔 <span className="text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400">{requests.length}</span></p>
                  ...
                  <div key={r.id} className="rounded-[20px] bg-white px-4 py-3.5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400 text-xs">친구 요청을 보냈어요</p>
                      </div>
                    </div>
          ...
          {friends.filter((f) => favoriteDocs[f.uid]).length > 0 && (
            <div>
              <p className="font-black text-slate-800 text-base mb-3 px-1">
                <svg className="inline-block w-4 h-4 mr-1 mb-0.5 text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400" viewBox="0 0 24 24" fill="currentColor" stroke="none">
                  <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                </svg>
                즐겨찾는 친구 <span className="text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400">{friends.filter((f) => favoriteDocs[f.uid]).length}</span>
              </p>
              <div className="space-y-2">
                  ...
                  <div key={f.uid} onClick={() => openProfile(f)} className="rounded-[20px] bg-sky-50 px-4 py-3.5 flex items-center justify-between cursor-pointer active:scale-[0.98] transition">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {mutedDocs[f.uid] && <span className="text-[10px] text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400 bg-sky-50 rounded-full px-2 py-0.5">🔕 알림 꺼짐</span>}
                      </div>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleFavorite(f); }}
                      className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-yellow-100 text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400"
                    >
          ...
          {visibleFriends.length > 0 && (
            <div>
              <p className="font-black text-slate-800 text-base mb-3 px-1">내 친구 👫 <span className="text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400">{visibleFriends.length}</span></p>
              <div className="space-y-2">
                {visibleFriends.map((f) => (
                  <div key={f.uid} onClick={() => openProfile(f)} className="relative rounded-[20px] bg-white px-4 py-3.5 flex items-center justify-between cursor-pointer active:scale-[0.98] transition">
                    ...
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-sky-50 text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400 font-black text-lg"
                    >···</button>
                ...
                return (
                  <div key={user.uid} className="rounded-[20px] bg-white px-4 py-3.5 flex items-center justify-between">
                    ...
                    {alreadyFriend ? (
                      ...
                      <span className="px-3 py-1.5 rounded-full bg-sky-50 text-transparent bg-clip-text bg-gradient-to-br from-sky-500 to-cyan-400 text-xs font-black">요청됨</span>
                    ) : (
