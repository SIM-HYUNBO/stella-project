  ...
  return (
    <main className="relative min-h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col px-4 py-4 gap-4 overflow-y-auto">
        {/* 제목 입력 */}
        <input
          className="rounded-[20px] bg-white  px-5 py-3.5 text-sm text-[gray-800] placeholder:text-[sky-400] outline-none font-semibold"
        ...
        {/* AI 요약 */}
        <div className={`rounded-[20px] border px-5 py-4 transition-all ${summary ? "bg-violet-500 border-purple-100" : "bg-white/80 border-sky-100"}`}>
        ...
        <div className="flex gap-3 pb-4">
          <button onClick={startListening} disabled={listening}
            className={`flex-1 h-12 rounded-[16px] font-black text-sm active:scale-[0.98] transition-transform ${listening ? "bg-yellow-100 text-gray-400" : "bg-green-500 text-white-[0_4px_14px_rgba(50,200,100,0.3)]"}`}>
            🎤 시작
          </button>
          <button onClick={stopListening} disabled={!listening}
            className={`flex-1 h-12 rounded-[16px] font-black text-sm active:scale-[0.98] transition-transform ${!listening ? "bg-yellow-100 text-gray-400" : "bg-red-500 text-white-[0_4px_14px_rgba(255,80,80,0.3)]"}`}>
            ⏹ 중지
          </button>
          <button onClick={summarizeMeeting} disabled={loading || !text}
            className={`flex-1 h-12 rounded-[16px] font-black text-sm active:scale-[0.98] transition-transform ${!text || loading ? "bg-yellow-100 text-gray-400" : "bg-sky-100 text-sky-800"}`}>
            ✨ 요약
          </button>
        </div>
      </div>
