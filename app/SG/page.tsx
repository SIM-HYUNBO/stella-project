  ...
  return (
    <main className="relative min-h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col px-4 py-4 gap-4 overflow-y-auto">
        ...
        {/* AI 요약 */}
        <div className={`rounded-[20px] border px-5 py-4 shadow-sm transition-all ${summary ? "bg-gradient-to-r from-violet-50 to-purple-50 border-purple-100" : "bg-white/80 border-orange-100"}`}>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-base">📌</span>
            <p className="font-black text-[#3d1f00] text-sm">AI 요약</p>
          </div>
        ...
        <div className="flex gap-3 pb-4">
          ...
          <button onClick={summarizeMeeting} disabled={loading || !text}
            className={`flex-1 h-12 rounded-[16px] font-black text-sm shadow-sm active:scale-[0.98] transition-transform ${!text || loading ? "bg-gray-200 text-gray-400" : "bg-gradient-to-r from-orange-400 to-amber-300 text-white shadow-[0_4px_14px_rgba(255,160,50,0.35)]"}`}>
            ✨ 요약
          </button>
        </div>
      </div>
