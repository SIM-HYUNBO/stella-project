  ...
  return (
    <main className="relative min-h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col px-4 py-4 gap-4 overflow-y-auto">
        {/* 제목 입력 */}
        <input
          className="rounded-[20px] bg-white border border-gray-300 px-5 py-3.5 text-sm text-[gray-800] placeholder:text-[sky-400] outline-none font-semibold shadow-sm"
        ...
        {/* 음성 텍스트 */}
        <div className="flex-1 min-h-[180px] rounded-[20px] bg-white border border-gray-300 px-5 py-4 shadow-sm overflow-auto">
        ...
        <div className="flex gap-3 pb-4">
          ...
          <button onClick={summarizeMeeting} disabled={loading || !text}
            className={`flex-1 h-12 rounded-[16px] font-black text-sm shadow-sm active:scale-[0.98] transition-transform ${!text || loading ? "bg-gray-200 text-gray-400" : "bg-sky-100 text-sky-800 shadow-sm"}`}>
            ✨ 요약
          </button>
        </div>
      </div>
