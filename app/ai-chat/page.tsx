    ...
    <main className="relative h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex items-center justify-between h-14 px-4 bg-white shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-black text-stone-800 text-sm">🧠 AI Chat</span>
        </div>
        <div className={`px-3 py-1 rounded-full text-xs font-black ${isVip ? "bg-yellow-100 text-white" : "bg-amber-50 text-amber-700"}`}>
          {isVip ? "💎 VIP" : "FREE"}
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {memory && isVip && (
          <div className="rounded-[16px] bg-yellow-100 px-4 py-3 text-xs text-amber-800 font-semibold">
        ...
        {messages.map((m: any, i: number) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} items-end gap-2`}>
            ...
            <div className={`px-4 py-2.5 rounded-[18px] max-w-[72%] text-sm leading-relaxed ${
              m.role === "user"
                ? "bg-amber-100 text-white rounded-br-[6px]"
                : "bg-white text-stone-800 rounded-bl-[6px]"
            }`}>
              {m.content}
            </div>
          </div>
