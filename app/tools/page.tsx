  ...
  return (
    <main className="relative min-h-screen bg-gray-50">
      ...
      <div className="flex items-center h-14 px-4 bg-white border-b-2 border-gray-700 sticky top-0 z-20">
        ...
        <span className="font-black text-[gray-800] text-base">⚙️ 설정</span>
      </div>

      <div className="px-5 pt-5 pb-20 space-y-4">

        {/* 내 정보 */}
        <div className="bg-white rounded-[20px] border-2 border-gray-700 shadow-sm px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              {editMode ? (
                <input
                  ...
                  className="w-full font-black text-[gray-800] text-base bg-sky-50 rounded-xl px-3 py-1.5 outline-none focus:ring-2 focus:ring-sky-200"
                  autoFocus
        ...
        {/* 앱 설정 토글 */}
        <div className="bg-white rounded-[20px] border-2 border-gray-700 shadow-sm overflow-hidden">
          ...
          {[
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-sky-500 flex items-center justify-center text-lg shadow-sm">
                  {icon}
                </div>
        ...
        <div className="bg-white rounded-[20px] border-2 border-gray-700 shadow-sm overflow-hidden">
          ...
          {navItems.map(({ icon, label, path }, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-sky-500 flex items-center justify-center text-lg shadow-sm">{icon}</div>
                <span className="font-bold text-[gray-800] text-sm">{label}</span>
              </div>
      ...
      {/* PIN 설정 모달 */}
      {showPinSetup && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-sky-500">
          ...
          <div className="flex gap-4 mb-3">
            {[0,1,2,3].map((i) => (
              <div key={i} className={`w-4 h-4 rounded-full border-2 transition-colors ${
                i < pinCurrent.length ? "bg-sky-500 border-sky-500" : "border-gray-300"
              }`} />
            ))}
          </div>
            ...
            {pinKeys.map((k, i) => (
              ...
              k === "←" ? (
                <button key={i} onClick={delPinKey}
                  className="h-16 rounded-2xl bg-white border-2 border-gray-700 shadow-sm text-2xl text-gray-500 flex items-center justify-center active:scale-95 transition">
                  ←
                </button>
              ) : (
                <button key={i} onClick={() => pressPinKey(k)}
                  className="h-16 rounded-2xl bg-white border-2 border-gray-700 shadow-sm text-2xl font-black text-[gray-800] flex items-center justify-center active:scale-95 active:bg-sky-50 transition">
                  {k}
                </button>
              )
            ))}
