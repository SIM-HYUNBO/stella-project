...
type StoryRequest = { id: string; from: string; to: string; status: string; sessionId: string };
...
export default function StoryLobbyPage() {
  ...
  const [incoming, setIncoming] = useState<StoryRequest[]>([]);
  const [outgoing, setOutgoing] = useState<StoryRequest | null>(null);
  ...
  useEffect(() => {
    if (!nickname) return;
    const q = query(collection(db, "story_requests"), where("to", "==", nickname), where("status", "==", "pending"));
    return onSnapshot(q, (snap) => {
      setIncoming(snap.docs.map(d => ({ id: d.id, ...d.data() } as StoryRequest)));
    });
  }, [nickname]);

  useEffect(() => {
    if (!nickname) return;
    const q = query(collection(db, "story_requests"), where("from", "==", nickname), where("status", "in", ["pending", "accepted"]));
    return onSnapshot(q, (snap) => {
      const accepted = snap.docs.find(d => d.data().status === "accepted");
      if (accepted) { router.push(`/creative/story/${accepted.data().sessionId}`); return; }
      const pending = snap.docs.find(d => d.data().status === "pending");
      setOutgoing(pending ? { id: pending.id, ...pending.data() } as StoryRequest : null);
    });
  }, [nickname]);
  ...
  const sendRequest = async (friend: Friend) => {
    ...
    await addDoc(collection(db, "story_requests"), {
  ...
  const acceptRequest = async (req: StoryRequest) => {
    await updateDoc(doc(db, "story_requests", req.id), { status: "accepted" });
    router.push(`/creative/story/${req.sessionId}`);
  };

  const rejectRequest = async (req: StoryRequest) => {
    await updateDoc(doc(db, "story_requests", req.id), { status: "rejected" });
  };

  const cancelRequest = async () => {
    if (!outgoing) return;
    await updateDoc(doc(db, "story_requests", outgoing.id), { status: "rejected" });
  };
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: "linear-gradient(160deg, #fdf4ff 0%, #fef9ec 50%, #f0f9ff 100%)" }}>
      <div className="flex items-center gap-3 px-5 pt-12 pb-6">
        <button onClick={() => router.back()} className="w-9 h-9 rounded-full bg-white/80 border border-purple-100 flex items-center justify-center text-slate-600 text-xl font-bold active:scale-90 transition-transform">‹</button>
        <div>
          <h1 className="font-black text-slate-800 text-xl">스토리 이어쓰기</h1>
          <p className="text-slate-400 text-xs">함께 이야기를 만들어요</p>
        </div>
      </div>
      ...
      <div className="px-5 space-y-5">
        {outgoing && (
          <div className="rounded-[20px] bg-yellow-50 border border-yellow-200 px-5 py-4 flex items-center justify-between">
            ...
            <button onClick={cancelRequest} className="text-xs text-red-400 font-bold px-3 py-1.5 rounded-full bg-red-50 active:scale-95 transition">취소</button>
          </div>
        )}
            ...
            {incoming.map((req) => (
              <div key={req.id} className="rounded-[20px] bg-white/80 border border-purple-200 px-5 py-4 flex items-center gap-4 shadow-sm">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-400 text-xs">같이 이야기 써요!</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => rejectRequest(req)} className="text-xs text-red-400 font-bold px-3 py-1.5 rounded-full bg-red-50 active:scale-95 transition">거절</button>
                  <button onClick={() => acceptRequest(req)} className="text-xs text-white font-bold px-3 py-1.5 rounded-full active:scale-95 transition" style={{ background: "linear-gradient(135deg, #a855f7, #ec4899)" }}>수락</button>
                </div>
              </div>
            ))}
          </div>
          ...
          {friends.length === 0 ? (
            ...
            <button key={friend.uid} onClick={() => sendRequest(friend)}
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-purple-400 text-xs">함께 이야기 쓰기 요청</p>
              </div>
