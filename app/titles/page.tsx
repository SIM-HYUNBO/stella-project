...
const ALL_TITLES = [
  ...
  { id: "groupmaster", icon: "🎪", name: "방장",        desc: "단체 채팅방 5개 이상 참여",      condition: "단체방 5개",          color: "from-sky-400 to-cyan-300" },
  { id: "nightowl",    icon: "🦉", name: "야행성",      desc: "자정 이후 메시지 보냄",          condition: "자정 이후 접속",      color: "from-slate-500 to-gray-600" },
  { id: "legend",      icon: "🌟", name: "레전드",      desc: "모든 칭호 10개 이상 획득",       condition: "칭호 10개",           color: "from-sky-500 via-pink-400 to-violet-400" },
];
            ...
            <div className="grid grid-cols-2 gap-3">
              {earnedTitles.map((t) => (
                <button key={t.id} onClick={() => equipTitle(t.id)}
                  className={`rounded-[20px] overflow-hidden active:scale-95 transition-transform ${equipped === t.id ? "ring-2 ring-sky-200 ring-offset-2" : ""}`}>
