...
const ALL_TITLES = [
  ...
  { id: "chatterer",   icon: "🗣️", name: "채팅왕",      desc: "메시지 200개 이상 보냄",         condition: "메시지 200개",        color: "bg-sky-200" },
  { id: "talkmaster",  icon: "👑", name: "말왕",        desc: "메시지 500개 이상 보냄",         condition: "메시지 500개",        color: "from-yellow-1000" },
  { id: "talkgod",     icon: "⚡", name: "말신",        desc: "메시지 1000개 이상 보냄",        condition: "메시지 1000개",       color: "from-sky-600" },
  { id: "friendly",    icon: "🤝", name: "친화력 갑",   desc: "친구 10명 이상",                 condition: "친구 10명",           color: "from-sky-400" },
  { id: "richfriend",  icon: "💎", name: "친구부자",    desc: "친구 50명 이상",                 condition: "친구 50명",           color: "from-cyan-500" },
  ...
  { id: "groupmaster", icon: "🎪", name: "방장",        desc: "단체 채팅방 5개 이상 참여",      condition: "단체방 5개",          color: "bg-yellow-200" },
  { id: "nightowl",    icon: "🦉", name: "야행성",      desc: "자정 이후 메시지 보냄",          condition: "자정 이후 접속",      color: "from-slate-500" },
  { id: "legend",      icon: "🌟", name: "레전드",      desc: "모든 칭호 10개 이상 획득",       condition: "칭호 10개",           color: "bg-sky-200" },
];
      ...
      <div className="px-4 pt-5 pb-24 space-y-6">
        ...
        <div className={`rounded-[28px] overflow-hidden`}>
          ...
          <div className="bg-white px-5 py-3 flex items-center justify-between">
            <p className="text-sm text-gray-500 font-semibold">획득한 칭호 <span className="text-sky-600 font-black">{earnedTitles.length}</span> / {titles.length}</p>
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            <div className="w-10 h-10 rounded-full border-4 border-sky-300 border-t-orange-400 animate-spin" />
          </div>
        )}
