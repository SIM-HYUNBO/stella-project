          ...
          {list.length === 0 ? (
            ...
            <div key={v.id} className="rounded-[20px] bg-white border-2 border-gray-700 px-5 py-4 flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-sky-500 flex items-center justify-center text-xl">🙈</div>
                <p className="font-black text-[gray-800] text-sm">{v.target_name}</p>
              </div>
              <button onClick={() => remove(v.id)} className="px-4 py-2 bg-gray-50 border-2 border-gray-700 text-sky-500 rounded-[12px] text-xs font-black">숨김 해제</button>
            </div>
          ))}
        </div>
      </div>
