        ...
        <div className="px-5 pt-6 pb-16 space-y-3">
          {list.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sky-400 font-semibold">숨긴 친구가 없어요</p>
            </div>
          ) : list.map((v) => (
            <div key={v.id} className="rounded-[20px] bg-white border border-gray-100 px-5 py-4 flex items-center justify-between shadow-sm">
              ...
              <button onClick={() => remove(v.id)} className="px-4 py-2 bg-gray-50 border border-gray-100 text-sky-500 rounded-[12px] text-xs font-black">숨김 해제</button>
            </div>
          ))}
        </div>
      </div>
