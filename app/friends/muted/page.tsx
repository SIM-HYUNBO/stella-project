        ...
        <div className="px-5 pt-6 pb-16 space-y-3">
          {list.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sky-600 font-semibold">알림을 끈 친구가 없어요</p>
            </div>
          ) : list.map((v) => (
            <div key={v.id} className="rounded-[20px] bg-white px-5 py-4 flex items-center justify-between">
              ...
              <button onClick={() => remove(v.id)} className="px-4 py-2 bg-gray-50 text-sky-600 rounded-[12px] text-xs font-black">알림 켜기</button>
            </div>
          ))}
        </div>
      </div>
