    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex items-center h-14 px-4 bg-white sticky top-0 z-20">
          ...
          <span className="font-black text-stone-800 text-base">🔕 알림 끈 친구</span>
        </div>
        <div className="px-5 pt-6 pb-16 space-y-3">
          {list.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-4xl mb-3">🔔</p>
              <p className="text-amber-700 font-semibold">알림을 끈 친구가 없어요</p>
            </div>
          ) : list.map((v) => (
            <div key={v.id} className="rounded-[20px] bg-white px-5 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <p className="font-black text-stone-800 text-sm">{v.target_name}</p>
              </div>
              <button onClick={() => remove(v.id)} className="px-4 py-2 bg-[#FFFBF0] text-amber-700 rounded-[12px] text-xs font-black">알림 켜기</button>
            </div>
          ))}
        </div>
      </div>
