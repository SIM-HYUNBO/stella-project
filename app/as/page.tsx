    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex items-center h-14 px-4 bg-white border-b-2 border-gray-700 sticky top-0 z-20">
          ...
          <span className="font-black text-[gray-800] text-base">🧠 AI 요약</span>
        </div>
        <div className="px-5 pt-6 pb-16 space-y-4">

          {/* 설명 카드 */}
          <div className="rounded-[24px] bg-white border-2 border-gray-700 px-5 py-5 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-12 h-12 rounded-xl bg-violet-500 flex items-center justify-center text-2xl shadow-md">🧠</div>
              <p className="font-black text-[gray-800] text-base">AI 요약 기능</p>
            </div>
          ...
          {/* 토글 */}
          <div className="rounded-[24px] bg-white border-2 border-gray-700 px-5 py-4 flex items-center justify-between shadow-sm">
            <div>
              <p className="font-black text-[gray-800] text-base">AI 요약 사용</p>
              <p className={`text-xs font-semibold mt-0.5 ${enabled ? "text-green-500" : "text-[sky-500]"}`}>
                {enabled ? "ON · 활성화됨" : "OFF · 비활성화됨"}
              </p>
            </div>
            <button onClick={toggle}
              className={`w-14 h-7 rounded-full transition-all duration-300 flex items-center px-0.5 ${enabled ? "bg-sky-500 shadow-md" : "bg-gray-200"}`}>
