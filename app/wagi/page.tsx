    ...
    <div style={{
      ...
      {!ended && (
        <div style={{
          ...
          borderRadius: "50%", background: "rgba(139,92,246,0.15)", filter: "blur(30px)",
          pointerEvents: "none",
        }} />
      )}
      ...
      <div style={{ height: 6, borderRadius: 99, background: "rgba(255,255,255,0.06)", overflow: "hidden", marginBottom: 8 }}>
        <div style={{
          ...
          background: ended ? "#374151" : "linear-gradient(90deg, #7c3aed, #a855f7)",
          transition: "width 0.5s",
          boxShadow: ended ? "none" : "0 0 8px rgba(168,85,247,0.6)",
        }} />
      </div>

      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <span style={{ fontSize: 11, color: "#6b7280" }}>
          {new Date(m.startAt).toLocaleDateString("ko-KR", { month: "short", day: "numeric" })} ~{" "}
          {new Date(m.endAt).toLocaleDateString("ko-KR", { month: "short", day: "numeric" })}
        </span>
        <span style={{ fontSize: 11, fontWeight: 800, color: ended ? "#555" : "#a78bfa" }}>
          {ended ? "종료됨" : formatLeft(m.endAt)}
        </span>
      </div>
    </div>
  );
...
export default function WagiPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (!snap.exists() || snap.data().isWagi !== true) {
        router.replace("/home");
        return;
      }
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "linear-gradient(160deg, #04000f 0%, #0d0020 50%, #04000f 100%)", color: "#e9d5ff", paddingBottom: 80 }}>
      {/* 배경 블롭 */}
      ...
      {/* 헤더 */}
      <div style={{ position: "relative", zIndex: 1, padding: "56px 20px 24px", textAlign: "center" }}>
        ...
        <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.3em", color: "#7c3aed", marginBottom: 8 }}>SECRET ROOM</p>
        <h1 style={{ fontSize: 24, fontWeight: 900, margin: 0, color: "#e9d5ff" }}>⚜️ 와기의 비밀방</h1>
        <p style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>와기 정식 회원만 입장 가능한 공간</p>
      </div>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "0 16px", maxWidth: 480, margin: "0 auto" }}>
        ...
        {missions.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {missions.map((m) => (
              <MissionCard key={m.id} m={m} isStella={isStella}
                onDelete={() => deleteDoc(doc(db, "wagi_missions", m.id))} />
            ))}
          </div>
        )}
      </div>
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }}
          ...
          <div style={{
            ...
            background: "#0d0020", border: "1.5px solid rgba(139,92,246,0.3)",
            ...
            <button onClick={addMission} disabled={saving || !form.title.trim() || !form.startAt || !form.endAt}
              style={{
                ...
                background: "linear-gradient(135deg, #6d28d9, #9333ea)",
                color: "#f3e8ff", fontWeight: 900, fontSize: 15,
