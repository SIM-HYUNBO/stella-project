...
export default function MeetingRoomPage() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(`chatTheme_meeting_${currentRoom.id}`);
  ...
  const renderRoom = () => (
    <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {showThemePicker && (
              <div className="fixed top-[56px] right-4 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/80 p-2.5 flex gap-2 z-[9999]" style={{ boxShadow: "0 8px 40px rgba(0,0,0,0.12)" }}>
                {THEMES.map(t => {
                  const a = ACCENT[t.id];
                  return (
                  <button key={t.id} title={t.id} onClick={() => { setActiveTheme(t.id); if (currentRoom?.id) localStorage.setItem(`chatTheme_meeting_${currentRoom.id}`, t.id); setShowThemePicker(false); }}
      ...
      {/* 메시지 */}
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-5 flex flex-col gap-3 relative" onClick={() => setShowThemePicker(false)}>
            ...
            <div key={m.id}>
              ...
              <div className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[80%]">
                  ...
                  <div className={`px-4 py-2.5 text-sm leading-relaxed font-medium ${
                    isMine
