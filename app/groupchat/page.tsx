      ...
      {showThemePicker && (
        <div className="fixed top-[56px] right-4 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/80 p-2.5 flex gap-2 z-[9999]"
          ...
          {THEMES.map(t => {
            const a = ACCENT[t.id];
            return (
              <button key={t.id} title={t.id} onClick={() => { setActiveTheme(t.id); if (currentRoom?.id) localStorage.setItem(`chatTheme_group_${currentRoom.id}`, t.id); setShowThemePicker(false); }}
