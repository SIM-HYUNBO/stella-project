    ...
    <main className="relative h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ...
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} items-end gap-2`}>
            ...
            <div className={`px-4 py-2.5 rounded-[18px] max-w-[72%] text-sm leading-relaxed shadow-sm ${
              m.role === "user"
                ...
                : "bg-white border border-gray-300 text-[gray-800] rounded-bl-[6px]"
            }`}>
              {m.content}
            </div>
          </div>
        ...
        {loading && (
          <div className="flex justify-start items-end gap-2">
            <div className="w-8 h-8 rounded-full bg-violet-500 flex items-center justify-center text-sm shrink-0 shadow-sm">🧠</div>
            <div className="px-4 py-3 rounded-[18px] rounded-bl-[6px] bg-white border border-gray-300 shadow-sm">
      ...
      <div className="relative z-10 flex items-center gap-2 px-4 py-3 bg-white border-t border-gray-300 shrink-0">
        ...
        <button onClick={sendMessage} disabled={loading}
          className="w-11 h-11 rounded-[14px] bg-sky-100 text-sky-800 font-black text-sm shadow-sm active:scale-95 transition-transform flex items-center justify-center">
          {loading ? "·" : "▶"}
        </button>
      </div>
    </main>
