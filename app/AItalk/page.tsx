  ...
  return (
    <main className="relative h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex items-center h-14 px-4 bg-white  shrink-0">
        ...
        <div className="w-9 h-9 rounded-full bg-violet-500 flex items-center justify-center text-lg mr-2">🧠</div>
        <div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} items-end gap-2`}>
            {m.role === "assistant" && (
              <div className="w-8 h-8 rounded-full bg-violet-500 flex items-center justify-center text-sm shrink-0">🧠</div>
            )}
            <div className={`px-4 py-2.5 rounded-[18px] max-w-[72%] text-sm leading-relaxed ${
              m.role === "user"
                ? "bg-sky-100 text-sky-800 rounded-br-[6px]"
                : "bg-white  text-[gray-800] rounded-bl-[6px]"
            }`}>
              {m.content}
            </div>
          </div>
          ...
          <div className="flex justify-start items-end gap-2">
            ...
            <div className="px-4 py-3 rounded-[18px] rounded-bl-[6px] bg-white ">
              <div className="flex gap-1">
                <div className="w-2 h-2 rounded-full bg-sky-100 animate-bounce" style={{ animationDelay: "0ms" }} />
                <div className="w-2 h-2 rounded-full bg-sky-100 animate-bounce" style={{ animationDelay: "150ms" }} />
                <div className="w-2 h-2 rounded-full bg-sky-100 animate-bounce" style={{ animationDelay: "300ms" }} />
              </div>
            </div>
          </div>
        )}
      ...
      <div className="relative z-10 flex items-center gap-2 px-4 py-3 bg-white  shrink-0">
        ...
        <button onClick={sendMessage} disabled={loading}
          className="w-11 h-11 rounded-[14px] bg-sky-100 text-sky-800 font-black text-sm active:scale-95 transition-transform flex items-center justify-center">
          {loading ? "·" : "▶"}
        </button>
      </div>
    </main>
