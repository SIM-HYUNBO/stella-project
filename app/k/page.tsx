    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-screen">
        <div className="flex items-center h-14 px-4 bg-white sticky top-0 z-20 shrink-0">
          ...
          <span className="font-black text-stone-800 text-base">📝 메모장</span>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden gap-3 p-4">
          ...
          <div className="flex-1 flex flex-col rounded-[24px] bg-white overflow-hidden">
            <div className="px-4 py-3 border-b border-amber-50 flex items-center gap-2">
              ...
              <p className="font-black text-stone-800 text-sm">{selectedId ? "메모 수정" : "새 메모"}</p>
              {selectedId && (
                <button onClick={() => { setText(""); setSelectedId(null); }}
                  className="ml-auto text-xs text-amber-700 font-semibold">취소</button>
              )}
            </div>
            <textarea value={text} onChange={(e) => setText(e.target.value)}
              className="flex-1 resize-none bg-transparent px-4 py-3 text-sm text-stone-800 outline-none placeholder:text-stone-400 leading-7"
              style={{ backgroundImage: "repeating-linear-gradient(transparent, transparent 27px, rgba(56,189,248,0.08) 28px)" }}
              placeholder="여기에 내용을 입력하세요..." />
            <button onClick={handleSave}
              className="mx-4 mb-4 h-12 rounded-[16px] bg-amber-100 text-white font-black text-sm active:scale-[0.98] transition-transform">
          ...
          <div className="w-40 flex flex-col gap-2 overflow-y-auto">
            <p className="font-black text-stone-800 text-xs px-1 mb-1">메모 목록</p>
            {notes.length === 0 && <p className="text-stone-400 text-xs text-center py-4">아직 없어요</p>}
            {notes.map((note) => (
              <div key={note.id}
                className={`rounded-[16px] border px-3 py-3 cursor-pointer transition-all ${note.id === selectedId ? "bg-amber-100 border-transparent" : "bg-white/80 border-amber-100"}`}
                onClick={() => handleSelect(note.id)}>
                <p className={`text-xs font-semibold truncate ${note.id === selectedId ? "text-white" : "text-stone-800"}`}>
