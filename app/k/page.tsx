        ...
        <div className="flex flex-1 overflow-hidden gap-3 p-4">
          ...
          <div className="flex-1 flex flex-col rounded-[24px] bg-white overflow-hidden">
            <div className="px-4 py-3 border-b border-sky-50 flex items-center gap-2">
              ...
              {selectedId && (
                <button onClick={() => { setText(""); setSelectedId(null); }}
                  className="ml-auto text-xs text-sky-600 font-semibold">취소</button>
              )}
            </div>
            ...
            <button onClick={handleSave}
              className="mx-4 mb-4 h-12 rounded-[16px] bg-sky-200 text-white font-black text-sm active:scale-[0.98] transition-transform">
          ...
          <div className="w-40 flex flex-col gap-2 overflow-y-auto">
            ...
            {notes.map((note) => (
              <div key={note.id}
                className={`rounded-[16px] border px-3 py-3 cursor-pointer transition-all ${note.id === selectedId ? "bg-sky-200 border-transparent" : "bg-white/80 border-sky-200"}`}
